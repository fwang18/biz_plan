...
class DataFetcher(object):
	...
	def fetch_topics(self, topic_list, n):
		for topic in topic_list:
			self.fetch(n, 'images\\{}'.format(topic), 'likes_{}.csv'.format(topic),
				method='search', keyword=topic, overwrite=True)
	...
	def fetch(self, n, to_folder, likes_file, method='walk', keyword=None,
		...
		if method == 'walk': 
			...
			photos_xml = self.flickr.photos.search(extras='url_m, count_faves, viws',
	...
	def fetch_binary(self, n=50, negative_sample_rate=None, overwrite=True):
		...
		for photo in photos:
			...
			url = photo.get('url_m')
			if likes > 0:
				urllib.request.urlretrieve(url, "images\\pos\\{}.jpg".format(i))
			else:
