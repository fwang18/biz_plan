...
def insert_users(id, name, email, subsciber,sessions_to_date):
    """
    ...
    SQLCursor.execute(
        """
        INSERT INTO picturesque.users (id, name, email, subscriber, sessions_to_date) 
        VALUES (%s, %s, %s, %s, %s);
        """, (id, name, email, subsciber, sessions_to_date))
...
def insert_sessions(id, ts, photos, free_trials_remaining):
    ...
    SQLCursor.execute(
        """
        INSERT INTO picturesque.sessions (id, ts, photos, free_trials_remaining) 
        VALUES (%s, %s, %s, %s);
...
insert_sessions(id = v1, ts = ts, photos=photos, free_trials_remaining = free_trials_remaining)
